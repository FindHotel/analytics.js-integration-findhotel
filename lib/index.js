...
Segment.prototype.normalize = function(msg) {
  ...
  if (!ctx.campaign) {
    ctx.campaign = query ? Object.assign(getVcid(query), utm(query)) : getVcid('');
  }
...
/**
 * Factory function to get vcid from url or the session storage
 * @param {string} query
 */
function getVcid(query='') {
  const queryParams = new URLSearchParams(query);
  const vcid = sessionStorage.getItem("vcid") || queryParams.get("vcid");
  if(vcid) return {vcid}
  return {}
}